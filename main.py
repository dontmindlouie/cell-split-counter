...
if __name__ == "__main__":
    ...
    parser.add_argument("--review-deaths", dest="review_death_events", action="store_true", default=False, help="run vision review of DEATH events. OFF by default since 2026-08-05 -- cell_mcp.py reads only tracks.csv/lineage.csv/annotations.csv from the bundle, never candidates.csv (where review verdicts land), so running this is pure inference cost with no effect on what the researcher sees today. Turn on once something (e.g. check_against_golden_set) actually consumes it.")
    parser.add_argument("--review-splits", dest="review_split_events", action="store_true", default=False, help="run vision review of SPLIT events. OFF by default since 2026-08-05, same reasoning as --review-deaths above.")
    ...
    run(
        ...
        review_split_events=args.review_split_events,
    )

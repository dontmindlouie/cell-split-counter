...
def test_nearest_detection_reports_id_and_distance_in_um(fake):
    tid, dum = cell_mcp_server._nearest_detection(fake, 0, 100.0, 100.0)
    assert tid == 1 and dum == pytest.approx(0.0)
    tid, dum = cell_mcp_server._nearest_detection(fake, 0, 104.0, 100.0)
...
def test_nearest_detection_excludes_the_anchor(fake):
    ...
    tid, _ = cell_mcp_server._nearest_detection(fake, 0, 100.0, 100.0)
    assert tid == 1
    tid, dum = cell_mcp_server._nearest_detection(fake, 0, 100.0, 100.0, exclude=1)
    assert tid == 2 and dum == pytest.approx(5.0)


def test_nearest_detection_is_none_when_the_frame_has_nothing(fake):
    assert cell_mcp_server._nearest_detection(fake, 7, 100.0, 100.0) is None
    # ...and when excluding the only cell present leaves nothing.
    assert cell_mcp_server._nearest_detection(fake, 1, 100.0, 100.0, exclude=1) is None
...
def test_rejects_an_unknown_anchor_track(fake):
    with pytest.raises(ValueError, match="anchor track 999 not found"):
        cell_mcp_server.watch_location_over_time(fake, start_frame=0, end_frame=2, anchor_track_id=999)

...
def _mk(monkeypatch, tracks_rows, n_frames=400):
    monkeypatch.setattr(cell_mcp_server, "_tracks", lambda w: pd.DataFrame(tracks_rows))
...
def test_a_re_acquired_mother_folds_into_the_first_recording(monkeypatch):
    ...
    sites = cell_mcp_server._collapse_sites(w, _rows(1, 2, 3), cell_mcp_server._tracks(w))
    assert len(sites) == 1
...
def test_the_representative_is_the_longest_lived_recording(monkeypatch):
    ...
    w = _mk(monkeypatch, rows)
    sites = cell_mcp_server._collapse_sites(w, _rows(1, 2), cell_mcp_server._tracks(w))
...
def test_a_chain_does_not_walk_through_a_whole_lineage(monkeypatch):
    ...
    w = _mk(monkeypatch, rows)
    sites = cell_mcp_server._collapse_sites(w, _rows(1, 2, 3, 4, 5, 6), cell_mcp_server._tracks(w))
    assert len(sites) > 1, "a 250-minute chain must not become one site"
    for rep, others in sites.items():
        g = [rep, *others]
        sub = cell_mcp_server._tracks(w)
        lo = int(sub[sub.track_id.isin(g)].frame.min())
        hi = int(sub[sub.track_id.isin(g)].frame.max())
        assert cell_mcp_server._minutes_between(w, lo, hi) <= cell_mcp_server._SITE_MAX_SPAN_MIN
...
def test_a_bundle_without_the_columns_gets_no_fold_rather_than_a_wrong_one(monkeypatch):
    ...
    monkeypatch.setattr(cell_mcp_server, "_manifest", lambda w: {"pixel_size_um": 1.0})
    sites = cell_mcp_server._collapse_sites("W", _rows(1, 2), cell_mcp_server._tracks("W"))
    assert sites == {1: [], 2: []}

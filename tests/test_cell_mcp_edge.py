...
def test_edge_um_uses_the_nearest_of_all_four_sides(fake_well):
    ...
    assert cell_mcp._edge_um(fake_well, 512, 512) == pytest.approx(256.0)
    # Near the left edge: 10 px -> 5 um, and left is nearer than top.
    assert cell_mcp._edge_um(fake_well, 10, 400) == pytest.approx(5.0)
    # Near the bottom: 1024-1020 = 4 px -> 2 um.
    assert cell_mcp._edge_um(fake_well, 500, 1020) == pytest.approx(2.0)
    # Near the right edge -- the M14 case, a track at cx=1010 of 1024.
    assert cell_mcp._edge_um(fake_well, 1010, 500) == pytest.approx(7.0)
...
def test_edge_um_is_nan_when_the_manifest_lacks_dimensions(monkeypatch):
    ...
    monkeypatch.setattr(cell_mcp, "_manifest", lambda well: {"pixel_size_um": 0.5})
    v = cell_mcp._edge_um("fake", 512, 512)
...
def test_edge_um_scales_with_pixel_size(monkeypatch):
    ...
    assert cell_mcp._edge_um("fake", 10, 50) == pytest.approx(20.0)

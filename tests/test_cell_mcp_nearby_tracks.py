...
def test_a_real_pair_is_reported_as_coexisting(well):
    out = cell_mcp.list_nearby_tracks(well, track_id=1, radius_um=30)
...
def test_a_long_standing_neighbour_is_excluded_by_default(well):
    ...
    out = cell_mcp.list_nearby_tracks(well, track_id=1, radius_um=30)
    assert _row(out, 40) == ""
    out2 = cell_mcp.list_nearby_tracks(well, track_id=1, radius_um=30, new_only=False)
...
def test_an_empty_neighbourhood_says_so_rather_than_returning_a_bare_table(well):
    out = cell_mcp.list_nearby_tracks(well, track_id=1, radius_um=2)
